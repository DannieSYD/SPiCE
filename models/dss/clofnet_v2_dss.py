...
class EquivariantLayerNorm(torch.nn.Module):
    ...
    def forward(self, field):
        field = field.permute(0, 2, 1)  # (N, C, 3)
        field_mean = torch.mean(field, dim=1, keepdim=True)  # (N, 1, 3)
        field = field - field_mean
        field_norm = field.pow(2).sum(-1)  # (N, C)
        field_norm = torch.mean(field_norm, dim=1, keepdim=True)  # (N, 1)
        field_norm = (field_norm + self.eps).pow(-0.5)

        field_norm = field_norm * self.affine_weight
...
class ClofNet(torch.nn.Module):
    ...
    def __init__(
        ...
        self.v_ln = EquivariantLayerNorm(hidden_channels)
        self.reset_parameters()
    ...
    def preprocess(self, data):
        ...
        v_emb = torch.zeros(z_emb.shape[0], 3, z_emb.shape[1], device=z_emb.device)
    ...
    def block_call(self, i, z_emb, v_emb, data_dict, batch):
        edge_index = data_dict['edge_index']
        edgefeature = data_dict['edgefeature']
        edgeweight = data_dict['edgeweight']
        coff_i = data_dict['coff_i']
        z_emb = self.message_layers[i](z_emb, edge_index, edge_attr=edgefeature, edgeweight=edgeweight)
        m_ij = self.message_layers[i].m_ij
        v_emb_l = coff_i[..., None] * m_ij.unsqueeze(1)
        v_emb = v_emb + scatter(v_emb_l, edge_index[1], dim=0, reduce='mean')
        v_emb = self.v_ln(v_emb)
        return z_emb, v_emb

    def postprocess(self, v_emb, batch):
        inv_emb = v_emb.norm(dim=1, keepdim=False)
        s = scatter(inv_emb, batch, dim=0)
        return s
