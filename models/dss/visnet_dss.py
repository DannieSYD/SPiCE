...
class ViSNet_DSS(torch.nn.Module):
    def __init__(
        self,
        lmax: int = 1,
        vecnorm_type: Optional[str] = None,
    ...
    def build_blocks(self):
        self.blocks = self.vis_mp_layers
    ...
    def block_call(self, i, x, vec, data_dict, edge_attr):
        edge_index, edge_weight, edge_vec = data_dict['edge_index'], data_dict['edge_weight'], data_dict['edge_vec']
        block = self.blocks[i]
        if not block.last_layer:
            dx, dvec, dedge_attr = block(x, vec, edge_index, edge_weight, edge_attr, edge_vec)
            x = x + dx
            vec = vec + dvec
            edge_attr = edge_attr + dedge_attr
            return x, vec, edge_attr
        else:
            dx, dvec, _ = block(x, vec, edge_index, edge_weight, edge_attr, edge_vec)
            x = x + dx
            vec = vec + dvec
    ...
    def postprocess(self, data, x, vec, batch):
        x = self.output_model.pre_reduce(x, vec)
        x = x * self.std

        if self.prior_model is not None:
            x = self.prior_model(x, data.x[:, 0])
