...
def boltzmann_average(quantities, energies, k=8.617333262145e-5, temperature=298):
    ...
    return np.sum(
        softmax(-np.asarray(energies) / k / temperature) * np.asarray(quantities)
    )
...
def canonicalize_3d_mol(mol_smiles, mol_3d):
    ...
    def get_reindexing_map(g1, g2, use_node_features=True):
        ...
        assert (
            gm.is_isomorphic()
        )  # THIS NEEDS TO BE CALLED FOR gm.mapping to be initiated
        idx_map = gm.mapping
...
def mol_to_data_obj(mol, conf_idx=0):
    # atoms
    atom_features_list = []
    ...
    if len(mol.GetBonds()) <= 0:
        ...
        for bond in mol.GetBonds():
            ...
            edge_attr.append(edge_feature)
        edge_index = (
            torch.tensor(np.asarray(edge_index), dtype=torch.long).t().contiguous()
        )
